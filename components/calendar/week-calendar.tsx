"use client";

import { Button, Chip, Spinner } from "@heroui/react";
import { AnimatePresence, motion } from "framer-motion";
import { useMemo, useState } from "react";
import { EmptyState } from "@/components/empty-state";
import { IconCalendar, IconChevron, IconChevronRight } from "@/components/icons";
import { ScopeTabs } from "@/components/scope/scope-tabs";
import { useSearch, useWorkspace } from "@/components/workspace-provider";
import { SCOPE_DOT } from "@/lib/constants";
import { useIssues } from "@/hooks/use-issues";
import { CALENDAR_END_HOUR, CALENDAR_START_HOUR, HOUR_HEIGHT } from "@/lib/constants";
import {
  addDays,
  formatDayLabel,
  formatTimeRange,
  formatWeekRange,
  isSameDay,
  minutesSinceStart,
  startOfWeek,
  toDateKey,
} from "@/lib/dates";
import type { Issue } from "@/lib/types";

const HOURS = Array.from(
  { length: CALENDAR_END_HOUR - CALENDAR_START_HOUR },
  (_, index) => CALENDAR_START_HOUR + index,
);

function blockStyle(issue: Issue) {
  if (!issue.startAt || !issue.endAt) return null;
  const top = (minutesSinceStart(issue.startAt, CALENDAR_START_HOUR) / 60) * HOUR_HEIGHT;
  const duration =
    (new Date(issue.endAt).getTime() - new Date(issue.startAt).getTime()) / 3600000;
  const height = Math.max(duration * HOUR_HEIGHT, 28);
  return { top, height };
}

export function WeekCalendar() {
  const { search } = useSearch();
  const { openIssue, openCreate, activePersonaId, activeScope } = useWorkspace();
  const issues = useIssues(activePersonaId, search, activeScope);
  const [weekStart, setWeekStart] = useState(() => startOfWeek());
  const days = useMemo(() => Array.from({ length: 7 }, (_, index) => addDays(weekStart, index)), [weekStart]);
  const todayKey = toDateKey(new Date());
  const issuesByDay = useMemo(() => {
    const map = new Map<string, Issue[]>();
    for (const issue of issues ?? []) {
      if (!issue.startAt) continue;
      const key = toDateKey(new Date(issue.startAt));
      const list = map.get(key) ?? [];
      list.push(issue);
      map.set(key, list);
    }
    return map;
  }, [issues]);

  const scheduledThisWeek = useMemo(
    () =>
      days.flatMap((day) => {
        const key = toDateKey(day);
        const dayIssues = (issuesByDay.get(key) ?? []).slice().sort((a, b) => {
          if (!a.startAt || !b.startAt) return 0;
          return a.startAt.localeCompare(b.startAt);
        });
        return dayIssues.length > 0 ? [{ day, key, issues: dayIssues }] : [];
      }),
    [days, issuesByDay],
  );

  if (issues === undefined) {
    return (
      <div className="flex h-full items-center justify-center">
        <Spinner />
      </div>
    );
  }

  function createBlockAt(day: Date, hour: number) {
    const start = new Date(day);
    start.setHours(hour, 0, 0, 0);
    const end = new Date(start);
    end.setHours(hour + 1, 0, 0, 0);
    openCreate({
      startAt: start.toISOString(),
      endAt: end.toISOString(),
      status: "todo",
      kind: "event",
    });
  }

  function onGridClick(day: Date, event: React.MouseEvent<HTMLDivElement>) {
    const rect = event.currentTarget.getBoundingClientRect();
    const y = event.clientY - rect.top;
    const hourFloat = CALENDAR_START_HOUR + y / HOUR_HEIGHT;
    const hour = Math.min(CALENDAR_END_HOUR - 1, Math.max(CALENDAR_START_HOUR, Math.floor(hourFloat)));
    createBlockAt(day, hour);
  }

  /** Keyboard activation has no pointer position, so fall back to the next open hour. */
  function onGridKeyDown(day: Date, event: React.KeyboardEvent<HTMLDivElement>) {
    if (event.key !== "Enter" && event.key !== " ") return;
    if (event.target !== event.currentTarget) return;
    event.preventDefault();

    const now = new Date();
    const hour =
      isSameDay(now.toISOString(), day) &&
      now.getHours() >= CALENDAR_START_HOUR &&
      now.getHours() < CALENDAR_END_HOUR
        ? now.getHours()
        : CALENDAR_START_HOUR;

    createBlockAt(day, hour);
  }

  const weekHasBlocks = days.some((day) =>
    (issues ?? []).some((issue) => issue.startAt && isSameDay(issue.startAt, day)),
  );

  return (
    <div className="flex h-full flex-col">
      <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between md:px-5">
        <div className="flex flex-wrap items-center gap-2">
          <Button
            isIconOnly
            aria-label="Previous week"
            className="touch-target"
            size="md"
            variant="tertiary"
            onPress={() => setWeekStart(addDays(weekStart, -7))}
          >
            <IconChevron className="size-4" />
          </Button>
          <Button
            isIconOnly
            aria-label="Next week"
            className="touch-target"
            size="md"
            variant="tertiary"
            onPress={() => setWeekStart(addDays(weekStart, 7))}
          >
            <IconChevronRight className="size-4" />
          </Button>
          <Button size="md" variant="secondary" onPress={() => setWeekStart(startOfWeek())}>
            Today
          </Button>
          <p className="text-sm font-medium">{formatWeekRange(weekStart)}</p>
        </div>
        <p className="hidden text-xs text-muted sm:block">
          Click an empty slot to time-block a new issue
        </p>
      </div>
      <div className="px-4 md:px-5">
        <ScopeTabs compact className="max-w-md" />
      </div>

      <div className="min-h-0 flex-1 overflow-auto">
        {!weekHasBlocks && (
          <div className="px-4 pt-6 md:px-5">
            <EmptyState
              icon={<IconCalendar className="size-5" />}
              title="Nothing scheduled this week"
              description="Tap New event to time-block a task or add a meeting."
              actionLabel="New event"
              onAction={() => openCreate({ kind: "event", status: "todo" })}
            />
          </div>
        )}

        <div className="flex flex-col gap-5 px-4 py-4 md:hidden">
          {scheduledThisWeek.map(({ day, key, issues: dayIssues }) => (
            <section key={key}>
              <h3
                className={`mb-2 text-xs font-medium tracking-wide uppercase ${
                  key === todayKey ? "text-accent" : "text-muted"
                }`}
              >
                {formatDayLabel(day)}
              </h3>
              <div className="flex flex-col gap-2">
                {dayIssues.map((issue) => (
                  <button
                    key={issue.id}
                    type="button"
                    onClick={() => openIssue(issue.id)}
                    className="min-h-11 rounded-lg border border-l-2 border-border bg-surface/60 px-3 py-2.5 text-left transition-colors hover:border-accent/40 hover:bg-surface"
                    style={{ borderLeftColor: SCOPE_DOT[issue.scope] }}
                  >
                    <p className="text-xs text-muted">{formatTimeRange(issue.startAt, issue.endAt)}</p>
                    <p className="mt-0.5 text-sm font-medium">{issue.title}</p>
                  </button>
                ))}
              </div>
            </section>
          ))}
        </div>

        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            key={toDateKey(weekStart)}
            initial={{ opacity: 0, x: 24 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -24 }}
            transition={{ duration: 0.18, ease: "easeOut" }}
            className="hidden min-w-[860px] grid-cols-[64px_repeat(7,minmax(0,1fr))] border-t border-border md:grid"
          >
            <div />
            {days.map((day) => {
              const key = toDateKey(day);
              return (
                <div
                  key={key}
                  className={`border-l border-border px-2 py-2 text-center text-xs ${
                    key === todayKey ? "text-accent" : "text-muted"
                  }`}
                >
                  {formatDayLabel(day)}
                </div>
              );
            })}

            <div className="relative">
              {HOURS.map((hour) => (
                <div
                  key={hour}
                  className="pr-2 text-right text-xs text-muted"
                  style={{ height: HOUR_HEIGHT }}
                >
                  {hour}:00
                </div>
              ))}
            </div>

            {days.map((day) => {
              const dayIssues = issuesByDay.get(toDateKey(day)) ?? [];
              return (
                <div
                  key={toDateKey(day)}
                  role="button"
                  tabIndex={0}
                  aria-label={`Add a time block on ${formatDayLabel(day)}`}
                  className="relative cursor-pointer border-l border-border focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-accent"
                  style={{ height: HOURS.length * HOUR_HEIGHT }}
                  onClick={(event) => onGridClick(day, event)}
                  onKeyDown={(event) => onGridKeyDown(day, event)}
                >
                  {HOURS.map((hour) => (
                    <div
                      key={hour}
                      className="border-t border-border/60 transition-colors hover:bg-accent/5"
                      style={{ height: HOUR_HEIGHT }}
                    />
                  ))}
                  {dayIssues.map((issue) => {
                    const style = blockStyle(issue);
                    if (!style) return null;
                    const isEvent = issue.kind === "event";
                    return (
                      <motion.button
                        key={issue.id}
                        type="button"
                        style={{
                          top: style.top,
                          height: style.height,
                          minHeight: 44,
                          borderColor: `${SCOPE_DOT[issue.scope]}99`,
                        }}
                        className={`absolute right-1 left-1 min-h-11 overflow-hidden rounded-md border-2 px-1.5 py-1 text-left ${
                          isEvent ? "bg-indigo-300/20" : "bg-emerald-300/15"
                        }`}
                        whileHover={{ scale: 1.03, boxShadow: "0 4px 16px rgba(0,0,0,0.25)" }}
                        transition={{ type: "spring", stiffness: 500, damping: 30 }}
                        onClick={(event) => {
                          event.stopPropagation();
                          openIssue(issue.id);
                        }}
                      >
                        <p className="truncate text-xs font-medium">{issue.title}</p>
                        <p className="truncate text-xs text-muted">
                          {formatTimeRange(issue.startAt, issue.endAt)}
                        </p>
                      </motion.button>
                    );
                  })}
                </div>
              );
            })}
          </motion.div>
        </AnimatePresence>
      </div>

      <div className="flex flex-wrap gap-2 border-t border-border px-5 py-3">
        <Chip size="sm" variant="soft">
          {(issues ?? []).filter((issue) => issue.startAt).length} scheduled
        </Chip>
        <Chip size="sm" variant="soft">
          {(issues ?? []).filter((issue) => !issue.startAt && issue.status !== "done").length} unscheduled
        </Chip>
      </div>
    </div>
  );
}
